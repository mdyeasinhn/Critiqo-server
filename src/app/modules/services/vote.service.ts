 ...
 */
const addVote = async (reviewId: string, userId: string, voteType: VoteType) => {
...
const getVotes = async (reviewId: string) => {
    ...
    // Format vote counts
    const upvotes = voteCount.find(v => v.type === VoteType.UPVOTE)?._count.id || 0;
    const downvotes = voteCount.find(v => v.type === VoteType.DOWNVOTE)?._count.id || 0;
