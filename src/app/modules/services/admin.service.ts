...
import { ReviewStatus, UserRole, UserStatus, VoteType, PaymentStatus } from "@prisma/client";
...
const getDashboardStats = async () => {
    ...
    voteStats.forEach(stat => {
        if (stat.type === VoteType.UPVOTE) formattedVoteStats.upvotes = stat._count.id;
        if (stat.type === VoteType.DOWNVOTE) formattedVoteStats.downvotes = stat._count.id;
    });
 ...
 */
const getPendingReviews = async (paginationOptions: IPaginationOptions): Promise<IGenericResponse<any>> => {
...
const moderateReview = async (
    ...
    const newStatus = action === 'publish' ? ReviewStatus.PUBLISHED : ReviewStatus.UNPUBLISHED;
