...
const createReview = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!isAuthUser(req.user)) {
        return next(new ApiError(StatusCodes.UNAUTHORIZED, 'Invalid user credentials'));
    }
    ...
    try {
        ...
        if (error.code === 'P2002') {
            return next(new ApiError(StatusCodes.CONFLICT, 'A review with similar unique constraints already exists'));
        }
...
const getAllReviews = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (req.query.limit && isNaN(Number(req.query.limit))) {
            return next(new ApiError(StatusCodes.BAD_REQUEST, 'Limit must be a number'));
        }
        
        if (req.query.page && isNaN(Number(req.query.page))) {
            return next(new ApiError(StatusCodes.BAD_REQUEST, 'Page must be a number'));
        }
        ...
        if (error.name === 'PrismaClientKnownRequestError') {
            return next(new ApiError(StatusCodes.BAD_REQUEST, 'Invalid query parameters'));
        }
...
const getReviewById = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!id) {
            return next(new ApiError(StatusCodes.BAD_REQUEST, 'Review ID is required'));
        }
        ...
        if (!result) {
            return next(new ApiError(StatusCodes.NOT_FOUND, `Review with ID ${id} not found`));
        }
        ...
        if (error.name === 'PrismaClientKnownRequestError' && error.code === 'P2023') {
            return next(new ApiError(StatusCodes.BAD_REQUEST, 'Invalid review ID format'));
        }
...
const updateReview = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!result) {
            return next(new ApiError(StatusCodes.NOT_FOUND, `Review with ID ${id} not found or not owned by the user`));
        }
        ...
        if (error.message.includes('Unauthorized')) {
            return next(new ApiError(StatusCodes.FORBIDDEN, 'You are not authorized to update this review'));
        }
...
const deleteReview = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (error.message.includes('Unauthorized')) {
            return next(new ApiError(StatusCodes.FORBIDDEN, 'You are not authorized to delete this review'));
        }
...
const getUserReviews = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (error.name === 'PrismaClientKnownRequestError') {
            if (error.code === 'P2023') {
                return next(new ApiError(StatusCodes.BAD_REQUEST, 'Invalid user ID format'));
            }
        }
        next(error);
    }
...
const removeImage = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!imageUrl) {
            return next(new ApiError(StatusCodes.BAD_REQUEST, 'Image URL is required'));
        }
        
        const result = await ReviewService.removeImage(id, userId, imageUrl);
        
        if (!result) {
            return next(new ApiError(StatusCodes.NOT_FOUND, 'Review not found or image could not be removed'));
        }
        ...
        if (error.message.includes('Unauthorized') || error.message.includes('Permission denied')) {
            return next(new ApiError(StatusCodes.FORBIDDEN, 'You are not authorized to remove images from this review'));
        }
